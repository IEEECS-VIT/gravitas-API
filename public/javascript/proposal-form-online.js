angular
  ...
  .controller('FormCtrl', function($scope, $http, $timeout) {
    $scope.resPerson = ["VIT Faculty", "VIT Student", "External Professional", "VIT Alumni"];
    $scope.eventCat = ["Bioxyn", "Circuitrix", "School", "Applied", "Builtrix", "Online", "Workshop", "Premium", "Bits", "Robomania", "Manage", "Quiz"];
    $scope.eventTypes = ["Technical", "Non-Technical", "Others", "Management"];
    ...
    function init() {
      ...
      $scope.formdata.others=[{'description':'','cost':0}];
      $scope.stat = "";
    ...
    $scope.delOther = function(v){
        if($scope.formdata.others.length!=1){
            ...
            toast("Deleted 1 Other cost field!");
        }
        else{
            toast("Atleast one other cost field is required.",4000);
        }
    }
    ...
    $scope.sub = function() {
     console.log("sub() : called");
      ...
      if ($scope.formdata.event.org.hasOwnProperty('coll')) {
        ...
        $scope.formdata.event.org = $scope.formdata.event.org.coll;
      }

      if($scope.org == "chap"){
          $scope.formdata.event.org = [$scope.formdata.event.org.chapter];
      }

      if($scope.org == "club"){
          $scope.formdata.event.org = [$scope.formdata.event.org.club];
      }

      if ($scope.org == "Individual") {
        $scope.formdata.event.org = ['Individual'];
      }
